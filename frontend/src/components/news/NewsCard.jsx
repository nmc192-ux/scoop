...
import { Bookmark, BookmarkCheck, ExternalLink, Clock, Share2, Sparkles, Loader2 } from "lucide-react";
...
export default function NewsCard({ article, index = 0, size = "normal" }) {
  const { saveArticle, unsaveArticle, isArticleSaved, language } = useNewsStore();
  ...
  const handleSummarize = async (e) => {
    ...
    try {
      ...
      // No-op; we keep the card usable even if AI is unavailable.
    } finally {
        ...
        <div className={clsx("p-4", isUrdu && "text-right")}>
          ...
          {(summary || isSummarizing) && (
            <div className="mt-3 rounded-xl border border-brand-blue/20 bg-brand-blue/5 p-3">
              <div className="mb-1 flex items-center gap-1.5 text-xs font-semibold text-brand-blue">
                {isSummarizing ? <Loader2 size={12} className="animate-spin" /> : <Sparkles size={12} />}
                {isUrdu ? "AI خلاصہ" : "AI Summary"}
              </div>
              <p className={clsx("text-sm text-[var(--color-text-secondary)]", isUrdu && "urdu-text")}>
                {summary || (isUrdu ? "خلاصہ تیار ہو رہا ہے..." : "Generating summary...")}
              </p>
            </div>
          )}
      ...
      <div className="px-4 pb-3 flex items-center justify-between border-t border-[var(--color-border)] pt-2.5">
        <div className="flex items-center gap-1">
          <motion.button
            ...
            className="p-1.5 rounded-lg text-[var(--color-text-tertiary)] hover:text-brand-blue hover:bg-brand-blue/10 transition-colors disabled:opacity-50"
            title={isUrdu ? "AI خلاصہ" : "AI Summary"}
          >
